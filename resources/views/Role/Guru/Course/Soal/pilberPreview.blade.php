...
<head>
  ...
  <title>Preview Soal Pilihan Ganda</title>
...
<body class="bg-gray-100">
  ...
  >
    <div
      class="bg-white rounded-lg shadow-lg max-w-4xl w-full mx-4 sm:mx-6 md:mx-0"
      role="dialog"
      aria-modal="true"
      aria-labelledby="previewModalLabel"
    >
            ...
            @foreach ($soal->jawaban_soal as $index => $jawaban)
              ...
              >
                <input
                  ...
                  @if($jawaban->benar) checked @endif
                  disabled
                />
                <span class="ml-3 text-gray-800 font-medium">
                  {{ chr(65 + $index) }}. {{ $jawaban->jawaban }}
                </span>
              </label>
            @endforeach
          </form>
