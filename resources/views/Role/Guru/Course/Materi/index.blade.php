...
@section('content')
    <div class="w-full bg-white p-6 shadow-md">
        ...
        @foreach ($materi->sortBy('tanggal_materi') as $exam)
            <h4 class="text-lg font-semibold mb-2">{{ \Carbon\Carbon::parse($exam->tanggal_materi)->format('d M Y') }}</h4>
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="flex space-x-5 justify-end">
                    <form action="{{ route('Guru.Materi.destroy', $exam->id_materi) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus soal ini?');">
                        @csrf
                        @method('DELETE')
